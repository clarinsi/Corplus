...
import { WORD_CATEGORY } from "@/constants";
...
export default function WordClassesList({
    parsedFilters,
    currentLemma,
}: {
    ...
    const { data, isLoading, error } = useQuery<BiblFilter[]>({
        ...
        queryFn: async ({ signal }: { signal: AbortSignal }) =>
            axios
                .get(`/api/filters/wordclass?lemma=${currentLemma}&${searchParams.toString()}`, { signal, baseURL: process.env.NEXT_PUBLIC_BASE_URL })
                .then((res) => res.data),
    });
